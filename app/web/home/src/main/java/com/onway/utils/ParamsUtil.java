...
public class ParamsUtil {
	...
	public static String getCommonXml(String sign, String cer, String pay,
		...
		String xmlStr = VelocityHelper.getInstance().evaluate(params,
				commTemplate);
	 ...
	 * @return
	 */
	public static String getCtrlInfo(String websvrName, String websvrCode,
			String appForm, String keep, String requestTime) {
	...
	public String getValue(String name, String xml) {
		String start = "<" + name + ">";
		String end = "</" + name + ">";
		String value = xml.substring(xml.indexOf(start) + start.length(),
				xml.indexOf(end));
	...
	public String getValue1(String name, String xml) {
		String start = "<" + name + ">";
		String end = "</" + name + ">";
		String value = xml.substring(xml.indexOf(start),
				xml.indexOf(end) + end.length());
		// logger.info("标签名=" + name + " 标签为=" + value);
		return value;
	}
	
	public String getValueCDATA(String xml) {
        String start = "<![CDATA[";
        String end = "]]>";
        int startLength = xml.indexOf(start);
        int endLength = xml.indexOf(end);
        int length = start.length();
        String value = xml.substring(startLength + length, endLength);
        return value;
    }
